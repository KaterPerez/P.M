...
class Mper {
    ...
    function save() {
        ...
        // Método para guardar un nuevo usuario
        function edit() {
            $sql = "UPDATE usuario SET numdoc:numdoc, tipdoc:tipdoc, nomusu:nomusu, apeusu:apeusu,, telusu:telusu, pasusu:pasusu, dirusu:dirusu, 
                    edausu:edausu, genusu:genusu, codper:codper, fotper:fotper";
            $modelo = new conexion();
            $conexion = $modelo->get_conexion();
            $result = $conexion->prepare($sql);
            $numdoc = $this->getNumdoc();
    ...
    public function updateFoto() {
        ...
        $result->bindParam(':fotper', $this->fotper);
        $result->bindParam(':idusu', $this->idusu);
    
        try {
            $result->execute();
        } catch (PDOException $e) {
            throw new Exception("Error al actualizar la foto: " . $e->getMessage());
        }
    }
}
?>